...
public class AccountInformationControllerV1 : ControllerBase
{
        ...
        [Required, FromBody] UpdateDescriptionRequest request)
    {
        ...
        await services.users.SetUserDescription(
    ...
    {
        var result =
            await services.accountInformation.GetUserGender(
                safeUserSession.userId);
    ...
    public async Task SetGender(
        [Required, FromBody] UpdateGenderRequest request)
    {
        await services.accountInformation.SetUserGender(
            safeUserSession.userId,
            request.gender);
    }
    ...
    {
        var result =
            await services.accountInformation.GetUserAvatarPageStyle(
                safeUserSession.userId);
    ...
    public async Task SetAvatarPageStyle(
        [Required, FromBody] UpdateAvPageStyleRequest request)
    {
        await services.accountInformation.SetUserAvatarPageStyle(
            safeUserSession.userId,
            request.avatarPageStyle);
    }
    ...
    public async Task<dynamic> GetPromotionChannels(
        bool alwaysReturnUrls = false)
    {
        var conn =
            await services.accountInformation.GetUserConnections(
                safeUserSession.userId);

        // Пользователь ещё не создавал promotion channels.
        // Возвращаем пустые значения вместо HTTP 500.
        conn ??= new UserConnections();

        return BuildPromotionChannels(conn, alwaysReturnUrls);
    }

    [HttpGet("users/{userId:long}/promotion-channels")]
    [HttpGetBypass("/v1/users/{userId:long}/promotion-channels")]
    public async Task<dynamic> GetPromotionChannelsFromUserId(
        long userId,
        bool alwaysReturnUrls = false)
    {
        // Проверяем авторизацию.
        var _ = safeUserSession.userId;

        var conn =
            await services.accountInformation.GetUserConnections(userId);

        // Если записи нет — возвращаем пустые каналы.
        conn ??= new UserConnections();

        return BuildPromotionChannels(conn, alwaysReturnUrls);
    }

    private dynamic BuildPromotionChannels(
        UserConnections conn,
        bool alwaysReturnUrls)
    {
        var discordUrl =
            alwaysReturnUrls &&
            !string.IsNullOrEmpty(conn.discord) &&
            IsDigits(conn.discord)
        ...
        return new
        {
            ...
            twitter = !string.IsNullOrEmpty(conn.twitter)
                ? (alwaysReturnUrls ? "https://x.com/" : "@") + conn.twitter
                : null,

            discord = discordUrl,

            telegram = !string.IsNullOrEmpty(conn.telegram)
                ? (alwaysReturnUrls ? "https://t.me/" : "@") + conn.telegram
                : null,

            tiktok = !string.IsNullOrEmpty(conn.tiktok)
                ? (alwaysReturnUrls ? "https://tiktok.com/@" : "@") + conn.tiktok
                : null,

            youtube = !string.IsNullOrEmpty(conn.youtube)
                ? (alwaysReturnUrls ? "https://youtube.com/@" : "@") + conn.youtube
                : null,

            twitch = !string.IsNullOrEmpty(conn.twitch)
                ? (alwaysReturnUrls ? "https://twitch.tv/" : "@") + conn.twitch
                : null,

            github = !string.IsNullOrEmpty(conn.github)
                ? (alwaysReturnUrls ? "https://github.com/" : "@") + conn.github
                : null,

            roblox = !string.IsNullOrEmpty(conn.roblox)
                ? (alwaysReturnUrls
    ...
    // ------------------------------------------------------------
    // Set promotion channels
    // ------------------------------------------------------------

    [HttpPost("promotion-channels")]
    [HttpPostBypass("/v1/promotion-channels")]
    public async Task<dynamic> SetPromotionChannels(
        [Required, FromBody] UserConnections conn)
    {
        if (conn == null)
        ...
        if (conn.twitter != null &&
            (string.IsNullOrWhiteSpace(conn.twitter) ||
             !Regex.IsMatch(
                 conn.twitter,
                 "^[_A-Za-z][A-Za-z0-9_]{3,14}$")))
        {
            throw new BadRequestException(
                12,
                "Twitter handle is invalid.");
        }

        if (conn.youtube != null &&
                 ...
                 @"^(?!.*\.\.)(?!.*\.$)[A-Za-z0-9._]{2,24}$")))
        {
            throw new BadRequestException(
                17,
                "TikTok handle is invalid.");
        }

        if (conn.discord != null &&
                 ...
                 "^[a-z0-9._]{2,32}$")))
        {
            throw new BadRequestException(
                15,
                "Discord handle is invalid.");
        }

        if (conn.telegram != null &&
        ...
        if (conn.roblox != null &&
            (string.IsNullOrWhiteSpace(conn.roblox) ||
             !Regex.IsMatch(
                 conn.roblox,
                 "^(?=.{3,20}$)(?!.*__)(?!.*_$)(?!^_)[A-Za-z0-9_]+$")))
        {
            throw new BadRequestException(
                14,
                "ROBLOX handle is invalid.");
        }

        await services.accountInformation.SetUserConnections(
            safeUserSession.userId,
            conn);
    ...
    private bool IsDigits(string? input)
    {
