...
public class AvatarService : ServiceBase, IService {
    ...
    public async Task Update3DRenderModified(long userId, string avatarHash)
    {
        ...
        try {
            var thumbnail3DJson = await File.ReadAllTextAsync(Path.Combine(Configuration.ThumbnailsDirectory, $"{avatarHash}_thumbnail3d.json"));
        ...
        bool skipRender = false, BodyScales? scales = null)
    {
        ...
        if (!forceRedraw)
        {
            ...
            if (File.Exists(Path.Combine(Configuration.ThumbnailsDirectory, $"{avatarHash}_thumbnail.png")) &&
                File.Exists(Path.Combine(Configuration.ThumbnailsDirectory, $"{avatarHash}_thumbnail3d.json")) &&
                File.Exists(Path.Combine(Configuration.ThumbnailsDirectory, $"{avatarHash}_headshot.png")))
            {
        ...
        Console.WriteLine($"[AVATAR DEBUG] results: {result.Length}");
        Console.WriteLine($"[AVATAR DEBUG] headshot: {result[0]}");
        Console.WriteLine($"[AVATAR DEBUG] thumbnail: {result[1]}");

var headshotResult = result[0];
var thumbnailResult = result[1];
        Directory.CreateDirectory(Configuration.ThumbnailsDirectory);
        ...
        try
        {
            // TODO: consider resizing these 3D renders вЂ” they may be too large
